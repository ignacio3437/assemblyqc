...
| `hic_map_combinations`          | Space separated tags denoting assemblies and assembly combinations for HiC map construction. For example, "tag1 tag2:tag3". In "tag2:tag3", "tag2" is treated as the query when applying refsort. Leaving as null results in each assembly getting a separate HiC map. | `string`  |                                                   |          |        |
...
| `hic_assembly_mode`             | To set or not to set the assembly mode for YAHS juicer pre stage                                                                                                                                                                                                       | `boolean` |                                                   |          |        |
| `hic_juicer_tools_pre_ext_args` | Extra parameters for the JuicerTools pre stage                                                                                                                                                                                                                         | `string`  |                                                   |          |        |
